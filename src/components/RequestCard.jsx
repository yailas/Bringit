  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">Price: ${request.price}</p>
        
        <div className="space-y-2">
          <div className="flex items-center text-sm text-gray-600">
            <MapPin className="w-4 h-4 mr-2 text-bringit-500" />
            <span className="truncate">From: <span className="font-medium text-gray-900">{request.from}</span></span>
          </div>
          <div className="flex items-center text-sm text-gray-600">
            <MapPin className="w-4 h-4 mr-2 text-bringit-500" />
            <span className="truncate">To: <span className="font-medium text-gray-900">{request.to}</span></span>
          </div>
          <div className="flex items-center text-sm text-gray-600">
            <Calendar className="w-4 h-4 mr-2 text-bringit-500" />
            <span>Before: {request.date}</span>
          </div>
        </div>

        <button 
          onClick={() => makeOffer(request.id, { price: request.reward })}
